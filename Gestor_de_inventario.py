...
class Ventana_ingreso_de_datos:
    def Ingresar_datos_a_mysql():
        if ID_SERIE.get() =="" or MARCA.get() =="" or MODELO.get()=="" or TECNOLOGIA.get()=="" or PULGADAS.get()=="" or RESOLUCION.get()=="" or SMART_TV.get()=="" or ROKU.get()=="" or CANTIDAD.get()=="" or PRECIO.get()=="" or CATEGORIA.get()=="":
            ...
            sql ="insert into INVENTARIOTV(ID_SERIE,MARCA,MODELO,TECNOLOGIA,PULGADAS,RESOLUCION,SMART_TV,ROKU,CANTIDAD,PRECIO,PRECIO_DE_VENTA,VALOR_TOTAL_MERCANCIA,CATEGORIA) VALUES ('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(ID_SERIE.get(),MARCA.get(),MODELO.get(),TECNOLOGIA.get(),PULGADAS.get(),RESOLUCION.get(),SMART_TV.get(),ROKU.get(),CANTIDAD.get(),PRECIO.get(),b,total,CATEGORIA.get())
            cursor.execute(sql)
            connectio.commit()
    ...
    def ventana_de_ingreso():
        ...
        ID_SERIE_ENTRY = Entry(validate="key",textvariable=ID_SERIE,font=('Bahnschrift',10))
        ID_SERIE_ENTRY.place(x=150,y=30)

        MARCA_C = ttk.Combobox(pnt,values =["ONN","TCL","HISENSE","RCA","LG","JVC","SPECTER","WESTINGHOUSE","SAMSUMG","HONEYW","ARTCTIC_AIR","LASKO","MIDEA","VORNADO","WOOZOO"],width=17,textvariable=MARCA,font=('Bahnschrift',10))
        MARCA_C.place(x=150,y=90)

        MODELO_ENTRY = Entry(pnt,textvariable=MODELO,font=('Bahnschrift',10))
        MODELO_ENTRY.place(x=370,y=90)

        TECNOLOGIA_C = ttk.Combobox(pnt,values=["LED","QLED","OLED","NULA"],width=17,textvariable=TECNOLOGIA,font=('Bahnschrift',10))
        ...
        RESOLUCION_C = ttk.Combobox(pnt,values=["720P","1080P","2K","4K","NULA"],width=17,textvariable=RESOLUCION,font=('Bahnschrift',10))
        RESOLUCION_C.place(x=150,y=280)

        SMART_TV_C= ttk.Combobox(pnt,values=["SI","NO","NULA"],width=17,textvariable=SMART_TV,font=('Bahnschrift',10))
        SMART_TV_C.place(x=150,y=340)

        ROKU_C = ttk.Combobox(pnt,values=["SI","NO","NULA"],width=17,textvariable=ROKU,font=('Bahnschrift',10))
...
class ventana_de_actualizacion:
    ...
    def ventana_de_tabla():
        ...
        MARCA_C_ven = ttk.Combobox(pnt,values =["ONN","TCL","HISENSE","RCA","LG","JVC","SPECTER","WESTINGHOUSE","SAMSUMG","HONEYW","ARTCTIC_AIR","LASKO","MIDEA","VORNADO","WOOZOO"],width=17,textvariable=MARCA,font=('Bahnschrift',10))
        ...
        TECNOLOGIA_C_ven = ttk.Combobox(pnt,values=["LED","QLED","OLED","NULA"],width=17,textvariable=TECNOLOGIA,font=('Bahnschrift',10))
        ...
        ROKU_C_ven = ttk.Combobox(pnt,values=["SI","NO","NULA"],width=17,textvariable=ROKU,font=('Bahnschrift',10))
        ...
        RESOLUCION_C_ven = ttk.Combobox(pnt,values=["720P","1080P","2K","4K","NULA"],width=17,textvariable=RESOLUCION,font=('Bahnschrift',10))
        RESOLUCION_C_ven.place(x=850,y=300)

        SMART_TV_C_ven= ttk.Combobox(pnt,values=["SI","NO","NULA"],width=17,textvariable=SMART_TV,font=('Bahnschrift',10))
    ...
    def arbol_datos():
        ...
        arbol.column("#0", anchor=CENTER, width=10)
...
class venta_de_busqueda_mysql():
    ...
    def buscar_informacion():
        ...
        if valor_busqueda =="":
            ...
            query = "SELECT * FROM INVENTARIOTV WHERE MARCA LIKE %s ORDER BY PRECIO"
...
###########################################################CONFIGURACION DE ABANICOS###############################################################################################
ID_A = StringVar()
SKU =StringVar()
MARCA_A=StringVar()
MODELO_A=StringVar()
DESCRIPCION_A = StringVar()
CANTIDAD_A=StringVar()
PRECIO_A=StringVar()

class ventana_de_ingreso_de_inventario_abanicos:
    connectio = pymysql.connect(
    host='localhost',
    user='root',
    password='basededatos',
    ...
    def guardar():
        descripcion = text_box.get("1.0", tk.END)  # obtiene el texto del widget
        DESCRIPCION_A.set(descripcion)
        if SKU.get()=="" or MARCA_A.get()=="" or MODELO_A.get()==""or DESCRIPCION_A.get()=="" or CANTIDAD_A.get()=="" or PRECIO_A.get()=="":
            messagebox.showinfo(message="Existen espacio vacios")
        else:
            cursor = connectio.cursor()
            sql ="insert into Inventarioabanico(SKU,MARCA,MODELO,DESCRIPCION,CANTIDAD,PRECIO) VALUES ('{}','{}','{}','{}','{}','{}')".format(SKU.get(),MARCA_A.get(),MODELO_A.get(),DESCRIPCION_A.get(),CANTIDAD_A.get(),PRECIO_A.get())
            cursor.execute(sql)
            connectio.commit()
            messagebox.showinfo(message="Elementos enviados",title="Estado de ingreso de datos")
            #Ventana_ingreso_de_datos.limpiar(set)
            ventana_de_ingreso_de_inventario_abanicos.limpiar(set)
    def limpiar(str):
        ENTRY_SKU.delete(0,END)
        ENTRY_MARCA_A.delete(0,END)
        ENTRY_MODELO_A.delete(0,END)
        text_box.delete(1.0,tk.END)
        ENTRY_CANTIDAD_A.delete(0,END)
        ENTRY_PRECIO_A.delete(0,END)    
        
    def ingresar_infomacion():
         global LABEL_SKU
         global LABEL_MARCA_A
         global LABEL_MODELO_A
         global LABEL_DESCRIPCION_A
         global LABEL_CANTIDAD_A
         global LABEL_PRECIO_A
         menu.Eliminar_botones()
         #pnt.config(background="silver")
   
         #LABEL_ID_A= Label(pnt,text="ID: ",font=('Bahnschrift',10))
         #LABEL_ID_A.place(x=50,y=30)
         
         LABEL_SKU= Label(pnt,text="SKU: ",font=('Bahnschrift',10))
         LABEL_SKU.place(x=50,y=50)

         LABEL_MARCA_A= Label(pnt,text="MARCA: ",font=('Bahnschrift',10))
         LABEL_MARCA_A.place(x=50,y=100)
         
         LABEL_MODELO_A= Label(pnt,text="MODELO: ",font=('Bahnschrift',10))
         LABEL_MODELO_A.place(x=50,y=150)
         
         LABEL_DESCRIPCION_A= Label(pnt,text="DESCRIPCION: ",font=('Bahnschrift',10))
         LABEL_DESCRIPCION_A.place(x=200,y=200)
         
         LABEL_CANTIDAD_A= Label(pnt,text="CANTIDAD: ",font=('Bahnschrift',10))
         LABEL_CANTIDAD_A.place(x=50,y=400)
         
         LABEL_PRECIO_A= Label(pnt,text="PRECIO: ",font=('Bahnschrift',10))
         LABEL_PRECIO_A.place(x=50,y=450)
         
         

         global ENTRY_SKU
         global ENTRY_MARCA_A
         global ENTRY_MODELO_A
         global text_box
         global ENTRY_CANTIDAD_A
         global ENTRY_PRECIO_A
         global boton_guardar

         
         ENTRY_SKU = Entry(validate="key",textvariable=SKU,font=('Bahnschrift',10))
         ENTRY_SKU.place(x=115,y=50,width=300)
         
         ENTRY_MARCA_A = Entry(validate="key",textvariable=MARCA_A,font=('Bahnschrift',10))
         ENTRY_MARCA_A.place(x=115,y=100,width=300)
         
         ENTRY_MODELO_A = Entry(validate="key",textvariable=MODELO_A,font=('Bahnschrift',10))
         ENTRY_MODELO_A.place(x=115,y=150,width=300)
         
         text_box = tk.Text(pnt, width=50, height=10, bg='white', fg='black')
         text_box.place(x=50, y=225)

         ENTRY_CANTIDAD_A = Entry(validate="key",textvariable=CANTIDAD_A,font=('Bahnschrift',10))
         ENTRY_CANTIDAD_A.place(x=125,y=400,width=300)
         
         ENTRY_PRECIO_A = Entry(validate="key",textvariable=PRECIO_A,font=('Bahnschrift',10))
         ENTRY_PRECIO_A.place(x=125,y=450,width=300)

         boton_guardar = tk.Button(pnt, text="ENVIAR DATOS", command=ventana_de_ingreso_de_inventario_abanicos.guardar)
         boton_guardar.place(x=250,y=500)

         menubar = Menu(pnt)
         pnt.config(menu=menubar)
         filemenu = Menu(menubar, tearoff=0)
         filemenu.add_command(label="Inicio",command=menu.eliminar_contenido_de_invetario_abanicos)
         menubar.add_cascade(label="Menu", menu=filemenu)


         

class VENTANA_DE_ACTUALIZACION_DE_ABANICOS():

    def actualiza_datos():
        if  SKU_ENTRY_ven.get()=="" or MARCA_ENTY_ven.get()=="" or MODELO_ENTRY_ven.get()=="" or DESCRIPCION_ENTRY_ven.get()=="" or CANTIDAD_ENTRY_ven.get()=="" or PRECIO_ENTRY_ven.get()==""or PRECIO_ENTRY_ven.get()==""or ID_ENTRY_VEN.get()=="":
            messagebox.showinfo(message="Existen espacios vacios")
        else:
            ID_SQL = ID_ENTRY_VEN.get()
            SKU_SQL = SKU_ENTRY_ven.get()
            MARCA_SQL = MARCA_ENTY_ven.get()
            MODELO_SQL = MODELO_ENTRY_ven.get()
            DESCRIPCION_SQL = DESCRIPCION_ENTRY_ven.get()
            CANTIDAD_SQL = CANTIDAD_ENTRY_ven.get()
            PRECIO_SQL = PRECIO_ENTRY_ven.get()
            cursos = connectio.cursor()
            consulta = "UPDATE Inventarioabanico SET SKU = %s,MARCA= %s,MODELO= %s,DESCRIPCION= %s,CANTIDAD= %s,PRECIO= %s WHERE ID=%s"
            valores = (SKU_SQL,MARCA_SQL,MODELO_SQL,DESCRIPCION_SQL,CANTIDAD_SQL,PRECIO_SQL,ID_SQL)
            cursos.execute(consulta,valores)
            connectio.commit()

            messagebox.showinfo(message="Elementos actualizados",title="Estado de ingreso de datos")
            messagebox.showinfo(message="Es nesario reiniciar pagina",title="Ingreso exitoso")
            VENTANA_DE_ACTUALIZACION_DE_ABANICOS.limpiar()
            menu.eliminar_Contenido_de_actualizacion_de_abanicos()

    def limpiar():
        ID_ENTRY_VEN.delete(0,END)
        SKU_ENTRY_ven.delete(0,END)
        MARCA_ENTY_ven.delete(0,END)
        MODELO_ENTRY_ven.delete(0,END)
        DESCRIPCION_ENTRY_ven.delete(0,END)
        CANTIDAD_ENTRY_ven.delete(0,END)
        PRECIO_ENTRY_ven.delete(0,END)
        
    def ventana_de_tabla():

        global LABEL_SKU_EDITAR
        global LABEL_MARCA_A_EDITARV
        global LABEL_MODELO_A_EDITAR
        global LABEL_DESCRIPCION_A_EDITAR
        global LABEL_CANTIDAD_A_EDITAR
        global LABEL_PRECIO_A_EDITAR
        global LABEL_ID_A 

        global SKU_ENTRY_ven
        global MARCA_ENTY_ven
        global MODELO_ENTRY_ven
        global DESCRIPCION_ENTRY_ven
        global CANTIDAD_ENTRY_ven
        global PRECIO_ENTRY_ven
        global ID_ENTRY_VEN
        
        VENTANA_DE_ACTUALIZACION_DE_ABANICOS.arbol_De_datos()

        LABEL_ID_A =Label(pnt,text="ID: ",font=('Bahnschrift',10))
        LABEL_ID_A.place(x=50,y=275)

        LABEL_SKU_EDITAR =Label(pnt,text="SKU: ",font=('Bahnschrift',10))
        LABEL_SKU_EDITAR.place(x=50,y=300)

        LABEL_MARCA_A_EDITARV =Label(pnt,text="MARCA: ",font=('Bahnschrift',10))
        LABEL_MARCA_A_EDITARV.place(x=50,y=325)

        LABEL_MODELO_A_EDITAR =Label(pnt,text="MODELO: ",font=('Bahnschrift',10))
        LABEL_MODELO_A_EDITAR.place(x=50,y=350)

        LABEL_DESCRIPCION_A_EDITAR =Label(pnt,text="DESCRIPCION: ",font=('Bahnschrift',10))
        LABEL_DESCRIPCION_A_EDITAR.place(x=50,y=375)

        LABEL_CANTIDAD_A_EDITAR =Label(pnt,text="CANTIDAD: ",font=('Bahnschrift',10))
        LABEL_CANTIDAD_A_EDITAR.place(x=50,y=400)

        LABEL_PRECIO_A_EDITAR =Label(pnt,text="PRECIO: ",font=('Bahnschrift',10))
        LABEL_PRECIO_A_EDITAR.place(x=50,y=425)

        ID_ENTRY_VEN = Entry(pnt,textvariable=ID_A,font=('Bahnschrift',10))
        ID_ENTRY_VEN.place(x=150,y=275,width=250)

        ############################

        SKU_ENTRY_ven = Entry(pnt,textvariable=SKU,font=('Bahnschrift',10))
        SKU_ENTRY_ven.place(x=150,y=300,width=250)

        MARCA_ENTY_ven = Entry(pnt,textvariable=MARCA_A,font=('Bahnschrift',10))
        MARCA_ENTY_ven.place(x=150,y=325,width=250)

        MODELO_ENTRY_ven = Entry(pnt,textvariable=MODELO_A,font=('Bahnschrift',10))
        MODELO_ENTRY_ven.place(x=150,y=350,width=250)

        DESCRIPCION_ENTRY_ven = Entry(pnt,textvariable=DESCRIPCION_A,font=('Bahnschrift',10))
        DESCRIPCION_ENTRY_ven.place(x=150,y=375,width=250)

        CANTIDAD_ENTRY_ven = Entry(pnt,textvariable=CANTIDAD_A,font=('Bahnschrift',10))
        CANTIDAD_ENTRY_ven.place(x=150,y=400,width=250)

        PRECIO_ENTRY_ven = Entry(pnt,textvariable=PRECIO_A,font=('Bahnschrift',10))
        PRECIO_ENTRY_ven.place(x=150,y=425,width=250)
    def eliminar_datos():
        ID_SQL = ID_ENTRY_VEN.get()
        alerta = messagebox.askyesno("Advertencia","Estas seguro de eliminar de la tabla la columna con el id= "+ ID_SQL)
        if alerta:
            cursor = connectio.cursor()
            consulta ="DELETE FROM Inventarioabanico WHERE id = %s"
            valor = (ID_SQL)
            cursor.execute(consulta,valor)
            connectio.commit()
            messagebox.showinfo(message="Elemento eliminado",title="Estado de ingreso de datos")
    ...
    def arbol_De_datos():
        ...
        global arbol
        arbol = ttk.Treeview(pnt)
        arbol['columns'] = ('columna1', 'columna2', 'columna3','columna4','columna5','columna6')
        a = 70
        arbol.heading("#0", text='ID')
        arbol.column("#0", anchor=CENTER, width=6)
        arbol.heading('columna1', text='SKU')
        arbol.column('columna1', anchor=CENTER, width=70)
        arbol.heading('columna2', text='MARCA')
        arbol.column('columna2', anchor=CENTER, width=70)
        arbol.heading('columna3', text='MODELO')
        arbol.column('columna3', anchor=CENTER, width=100)
        arbol.heading('columna4', text='DESCRIPCION')
        arbol.column('columna4', anchor=CENTER, width=300)
        arbol.heading('columna5', text='CANTIDAD')
        arbol.column('columna5', anchor=CENTER, width=10)
        arbol.heading('columna6', text='PRECIO_DLLS')
        arbol.column('columna6', anchor=CENTER, width=10)
        arbol.place(x=35,y=25,width=1100,height=245)
        def mostrar_datos(event):
            item = arbol.selection()[0]
            values = arbol.item(item, 'values')

            ID_ENTRY_VEN.delete(0, END)
            ID_ENTRY_VEN.insert(0, arbol.item(item, 'text'))
            
            SKU_ENTRY_ven.delete(0, END)
            SKU_ENTRY_ven.insert(0, values[0])

            MARCA_ENTY_ven.delete(0, END)
            MARCA_ENTY_ven.insert(0, values[1])

            MODELO_ENTRY_ven.delete(0, END)
            MODELO_ENTRY_ven.insert(0, values[1])

            MODELO_ENTRY_ven.delete(0, END)
            MODELO_ENTRY_ven.insert(0, values[2])

            DESCRIPCION_ENTRY_ven.delete(0, END)
            DESCRIPCION_ENTRY_ven.insert(0, values[3])

            CANTIDAD_ENTRY_ven.delete(0, END)
            CANTIDAD_ENTRY_ven.insert(0, values[4])
            
            PRECIO_ENTRY_ven.delete(0, END)
            PRECIO_ENTRY_ven.insert(0, values[5])
            
        arbol.bind('<<TreeviewSelect>>', mostrar_datos) 
        
        mycursor = connectio.cursor()
        mycursor.execute("SELECT * FROM Inventarioabanico")
        result = mycursor.fetchall()
        for row in result:
            arbol.insert("", "end", text=row[0], values=(row[1], row[2], row[3], row[4], row[5], row[6]))
        connectio.close()

        global botonenviarven
        botonenviarven = Button(pnt,text="Actualizar",command=VENTANA_DE_ACTUALIZACION_DE_ABANICOS.actualiza_datos)
        botonenviarven.place(x=300,y=500)
        
        global botoneliminar
        botoneliminar = Button(pnt,text="Eliminar",command=VENTANA_DE_ACTUALIZACION_DE_ABANICOS.eliminar_datos)
        botoneliminar.place(x=700,y=500)
              
        global menubar
        menubar = Menu(pnt)
        pnt.config(menu=menubar)
        filemenu = Menu(menubar, tearoff=0)
        filemenu.add_command(label="Inicio",command=menu.eliminar_Contenido_de_actualizacion_de_abanicos)
        menubar.add_cascade(label="Menu", menu=filemenu)
        #################################################################################################### ############33
...
class venta_de_busqueda_mysql_a():

    def buscar_informacion_a():
        valor_busqueda = entry_busqueda.get()
        cnx = mysql.connector.connect(
            host='localhost',
            user='root',
        ...
        if valor_busqueda =="":
            ...
            query = "SELECT * FROM Inventarioabanico WHERE MARCA LIKE %s ORDER BY MARCA"
            cursor.execute(query, ('%' + valor_busqueda + '%',))
            resultados = cursor.fetchall()
            for i in arbol.get_children():
                arbol.delete(i)
            for resultado in resultados:
                arbol.insert("", END, text=resultado[0], values=(resultado[0], resultado[1], resultado[2], resultado[3], resultado[4], resultado[5], resultado[6]))
                cursor.close()
            cnx.close()
    ...
    def ventana_de_busqueda():
        ...
        global label_busqueda
        global boton_buscar
        label_busqueda = Label(pnt, text="Buscar ABANICOS por MODELO:")
        label_busqueda.pack()
        entry_busqueda = Entry(pnt)
        entry_busqueda.pack()
        boton_buscar = Button(pnt, text="Buscar", command=venta_de_busqueda_mysql_a.buscar_informacion_a)
        boton_buscar.pack()
        columnas = ("ID", "SKU", "MARCA","MODELO","DESCRIPCION","CANTIDAD","PRECIO")
        arbol = Treeview(pnt, columns=columnas, show="headings")
        for col in columnas:
            arbol.heading(col, text=col)
            arbol.column("ID",width=80)
            arbol.column("SKU",width=95)
            arbol.column("MARCA",width=70)
            arbol.column("MODELO",width=90)
            arbol.column("DESCRIPCION",width=70)
            arbol.column("CANTIDAD",width=80)
            arbol.column("PRECIO",width=70)
        arbol.pack()
        menubar = Menu(pnt)
        pnt.config(menu=menubar)
        filemenu = Menu(menubar, tearoff=0)
...
class menu:
    ...
    def eliminar_contenido_de_invetario_abanicos():
        LABEL_SKU.destroy()
        LABEL_MARCA_A.destroy()
        LABEL_MODELO_A.destroy()
        LABEL_DESCRIPCION_A.destroy()
        LABEL_CANTIDAD_A.destroy()
        LABEL_PRECIO_A.destroy()
        ENTRY_SKU.destroy()
        ENTRY_MARCA_A.destroy()
        ENTRY_MODELO_A.destroy()
        text_box.destroy()
        ENTRY_CANTIDAD_A.destroy()
        ENTRY_PRECIO_A.destroy()
        boton_guardar.destroy()
        menu.Eliminar_botones()
        menu.botones_principlaes(set)

    def eliminar_Contenido_de_actualizacion_de_abanicos():
        LABEL_SKU_EDITAR.destroy()
        LABEL_MARCA_A_EDITARV.destroy()
        LABEL_MODELO_A_EDITAR.destroy()
        LABEL_DESCRIPCION_A_EDITAR.destroy()
        LABEL_CANTIDAD_A_EDITAR.destroy()
        LABEL_PRECIO_A_EDITAR.destroy()
        LABEL_ID_A.destroy()

        SKU_ENTRY_ven.destroy()
        MARCA_ENTY_ven.destroy()
        MODELO_ENTRY_ven.destroy()
        DESCRIPCION_ENTRY_ven.destroy()
        CANTIDAD_ENTRY_ven.destroy()
        PRECIO_ENTRY_ven.destroy()
        ID_ENTRY_VEN.destroy()
        botonenviarven.destroy()
        botoneliminar.destroy()
        arbol.destroy()
        menu.botones_principlaes(set)
    ...
    def botones_principlaes(set):
        ...
        global Btn_ingreso_de_datos_ABANICOS
        global Btn_Actualizacion_de_datos_ABANICOS
        global btn_venta
        global Btn_Busqueda_de_datos_ABANICOS

        Btn_ingreso_de_datos = Button(pnt,text="AGREGAR INVENTARIO TV",command=Ventana_ingreso_de_datos.ventana_de_ingreso)
        Btn_ingreso_de_datos.place(x=200,y=100,width=200,height=100)

        Btn_Actualizacion_de_datos = Button(pnt,text="ACTUALIZAR INVENTARIO TV",command=ventana_de_actualizacion.ventana_de_tabla)
        Btn_Actualizacion_de_datos.place(x=450,y=100,width=200,height=100)

        Btn_Busqueda_de_datos = Button(pnt,text="BUSQUEDA DE INVENTARIO TV",command=venta_de_busqueda_mysql.ventana_de_busqueda)
        Btn_Busqueda_de_datos.place(x=700,y=100,width=208,height=100)

        

        Btn_ingreso_de_datos_ABANICOS =Button(pnt,text="AGREGAR INVENTARIO ABANICOS",command=ventana_de_ingreso_de_inventario_abanicos.ingresar_infomacion)
        Btn_ingreso_de_datos_ABANICOS.place(x=200,y=250,width=200,height=100)
        
        Btn_Actualizacion_de_datos_ABANICOS = Button(pnt,text="ACTUALIZAR INVENTARIO ABANICOS",command=VENTANA_DE_ACTUALIZACION_DE_ABANICOS.ventana_de_tabla)
        Btn_Actualizacion_de_datos_ABANICOS.place(x=450,y=250,width=208,height=100)

        Btn_Busqueda_de_datos_ABANICOS = Button(pnt,text="BUSQUEDA DE INVENTARIO ABANICO",command=venta_de_busqueda_mysql_a.ventana_de_busqueda)
        Btn_Busqueda_de_datos_ABANICOS.place(x=700,y=250,width=208,height=100)

        btn_venta = Button(pnt,text="VENTAS",command=venta_de_busqueda_mysql.ventana_de_busqueda)
        btn_venta.place(x=450,y=380,width=208,height=100)
    ...
    def Eliminar_botones():
        ...
        Btn_ingreso_de_datos_ABANICOS.destroy()
        Btn_Actualizacion_de_datos_ABANICOS.destroy()
        btn_venta.destroy()
        Btn_Busqueda_de_datos_ABANICOS.destroy()
